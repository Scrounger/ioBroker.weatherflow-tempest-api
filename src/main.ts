...
import { ForeCastCurrent, ForeCastDaily, ForeCastHourly } from './lib/api/wft-types-forecast.js';
...
class WeatherflowTempestApi extends utils.Adapter {
	...
	private async onStateChange(id: string, state: ioBroker.State | null | undefined): Promise<void> {
		...
		try {
			if (state && !state.ack) {
				if (id.endsWith(`.${tree.forecast.idChannel}.${(tree.forecast.get().update as myTreeState).id}`)) {
	...
	private async updateForeCast(isAdapterStart: boolean = false): Promise<void> {
	...
	private async updateForeCastCurrent(data: ForeCastCurrent, isAdapterStart: boolean = false): Promise<void> {
	...
	private async updateForeCastHourly(data: Array<ForeCastHourly>, isAdapterStart: boolean = false): Promise<void> {
	...
	private async updateForeCastDaily(data: Array<ForeCastDaily>, isAdapterStart: boolean = false): Promise<void> {
	...
	private async statesChanged(statesChanged: boolean, idLastUpdate: string, logPrefix: string): Promise<void> {
		if (statesChanged) {
